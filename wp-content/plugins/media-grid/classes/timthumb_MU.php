...
if(! defined('FILE_CACHE_SUFFIX') ) 		define ('FILE_CACHE_SUFFIX', '.mg_timthumb_cache.txt');			// What to put at the end of all files in the cache directory so we can identify them
if(! defined('FILE_CACHE_PREFIX') ) 		define ('FILE_CACHE_PREFIX', 'mg_timthumb');				// What to put at the beg of all files in the cache directory so we can identify them
...
class timthumb {
	...
	protected function getLocalImagePath($src){
		...
		if (is_multisite()){
			$pos = strrpos($src, '/');
			$new_src = substr($src, $pos);

			$path = $this->docRoot . '/files' . $new_src;
			return $path;
